...
namespace Identity.Services
{
    public class AccountService : IAccountService
    {
        ...
        public async Task<Response<AuthenticationResponse>> AuthenticateAsync(AuthenticationRequest request, string ipAddress)
        {
            ...
            var result = await _signInManager.PasswordSignInAsync(usuario, request.Password, false, lockoutOnFailure: false);
        ...
        {
            var usuarioMismoName = await _userManager.FindByNameAsync(request.UserName);

            if(usuarioMismoName != null)
            ...
            var usuarioMismoCorreo = await _userManager.FindByEmailAsync(request.Email);
        ...
        private async Task<JwtSecurityToken> GenerateJWTToken(ApplicationUser usuario)
        {
            ...
            var symmetricSecurtyKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(_jwtSettings.Key));
            var signInCredentials = new SigningCredentials(symmetricSecurtyKey, SecurityAlgorithms.HmacSha256);
